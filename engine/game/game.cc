...
//        ears -- implemention of $pref::Audio::usePlayerCentricListener preference. 
...
/*
//------------------------------------------------------------------
// TGE Modernization Kit: DARREN MOD> moved gameTSCtrl.cc codes to here
// GameReflectionClipPlane()
// GameReflectionSupport()
// GameStartReflectionRender()
// GameEndReflectionRender()
//------------------------------------------------------------------
...
// AFX CODE BLOCK (ears) <<
static bool sUsePlayerCentricListener = false; 
// AFX CODE BLOCK (ears) >>
} // namespace {}
...
void GameInit()
{
   ...
   // DARREN MOD <<
   Con::addVariable("$pref::zip::ext", TypeString, &ResManager::sZipExt);
   // DARREN MOD >>
...
//      alxListener3f(AL_VELOCITY, velocity.x, velocity.y, velocity.z);
   ...
   // AFX CODE BLOCK (ears) <<
   // Setting global script variable $pref::usePlayerCentricListener to true
   // replaces the usual camera-centric listener with a player-centric one.
   // In other words the ears are on the player instead of the camera. A subtle
   // distinction when using a 3rd person over-the-shoulder style camera.
   if (sUsePlayerCentricListener)
   {
     GameConnection* conn = GameConnection::getConnectionToServer();
     if (conn)
     {
        ShapeBase* ctrl_obj = conn->getControlObject();
        if (ctrl_obj)
        {
          MatrixF ctrl_obj_mtx = ctrl_obj->getRenderTransform();
          Point3F pt = ctrl_obj_mtx.getPosition();
          pt.z += 2.0f;
          ctrl_obj_mtx.setPosition(pt);
          alxListenerMatrixF(&ctrl_obj_mtx);
        }
     }
   }
   // AFX CODE BLOCK (ears) >>
...
/*
WaterBlock * GameReflectionSupport()
{
   SimpleQueryList sql;
   gClientSceneGraph->getWaterObjectList(sql);
   if (sql.mList.size()) //Only if we actually need a reflection 
   {
		return static_cast<WaterBlock*>(sql.mList[0]);
   }
   return 0;
}

void GameReflectionClipPlane(WaterBlock* pBlock, CameraQuery *camq, GLdouble clipPlane[])
{
	if (!pBlock) return;
	...
	mkFluid* fluidData = pBlock->getMKFluid();

	// Update reflect/refract in case global has changed
	fluidData->setReflectionSize(pBlock->getReflectionSize());
	fluidData->enableRefraction(pBlock->isRefractive());
	fluidData->prepReflection(camq, plane); 
	...
	clipPlane[1] = norm.y;
	clipPlane[2] = norm.z;
	clipPlane[3] = (pBlock->getSurfaceHeight()-0.5);
}

void GameStartReflectionRender(WaterBlock* pBlock, GLdouble clipPlane[])
{
   if (pBlock) 
   {
      if(!MKGFX->renderPlanarReflection())
         return;
...
void GameEndReflectionRender(WaterBlock* pBlock)
{
	if (!pBlock) return;

	glDisable(GL_CLIP_PLANE0);
	glCullFace(GL_BACK);
	MKGFX->endPlanarReflectionRender();
}
*/

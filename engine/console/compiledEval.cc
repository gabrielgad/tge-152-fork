         ...
         case OP_CALLFUNC:
         {
            ...
            else
            {
               if((nsEntry->mMinArgs && S32(callArgc) < nsEntry->mMinArgs) || (nsEntry->mMaxArgs && S32(callArgc) > nsEntry->mMaxArgs))
               {
                  ...
                  Con::warnf(ConsoleLogEntry::Script, "%s: %s::%s - wrong number of arguments.given[%d] min[%d] max[%d]", getFileLine(ip-4), nsName, fnName,callArgc,nsEntry->mMinArgs,nsEntry->mMaxArgs);

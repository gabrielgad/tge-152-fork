...
//----------------------------------------------------------------
bool  uniEncDec(char * buf, size_t size)
{
	U8 mask[] = {0xAA, 0xCC};
	U8 maskSize = sizeof(mask);
	U8 mId = 0; 

	for (size_t i = 0; i < size; ++i)
	{
		buf[i] ^= mask[mId++];
		if (mId >= maskSize) mId = 0;
	}

	return true;
}
bool  quadEncDec(U32 * buf, size_t size)
{
	U32 mask[] = {0xAABBFCDD, 0xD7EC56BF};
	U8 maskSize = sizeof(mask)/sizeof(mask[0]);
	U8 mId = 0; 
...
// Immediately remove deleted file from sandbox
...
ConsoleFunction(fileEncDec, bool, 3, 3, "fileEncDec(source, destination")
{
	FILE * pFile;
	U32 * buffer;

	pFile = fopen ( argv[1] , "rb" );
	if (pFile==NULL) {Con::errorf("fileEncDec(): File error"); return false;}

	// obtain file size:
	fseek (pFile , 0 , SEEK_END);
	const size_t bSize = ftell (pFile); // byte size
	rewind (pFile);

	const size_t uSize = sizeof(buffer[0]); // unit size
	const size_t sSize = (bSize+(uSize-1))/uSize; //scaled size
	const size_t aSize = uSize*sSize;

	// allocate memory to contain the whole file:
	buffer = (U32*) dRealMalloc (aSize);
	dMemset(buffer, 0, aSize);
	if (buffer == NULL) {Con::errorf("fileEncDec(): Memory error"); return false;}

	// copy the file into the buffer:
	if (fread (buffer,uSize,sSize,pFile) != bSize/4) {Con::errorf("fileEncDec(): Reading error"); return false;}
	...
	quadEncDec(buffer, sSize);

	pFile = fopen ( argv[2] , "wb" );
	if (pFile==NULL) {Con::errorf("fileEncDec(): File error"); dRealFree(buffer); return false;}
	fwrite (buffer , uSize , sSize, pFile );
	fclose (pFile);
